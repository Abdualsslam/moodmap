...
class MoodEmojis extends StatelessWidget {
  ...
  const MoodEmojis({super.key, required this.moodList, required this.selected, required this.onSelect});
      ...
      children:
          moodList.map((mood) {
            ...
            Widget emoji = AnimatedContainer(
              ...
              child: Image.asset(mood.assetPath, height: 45, width: 45, fit: BoxFit.contain),
            );

            // لو الإيموجي هو المختار فعّل الحركة
            if (isSelected) {
              emoji = AnimatedScale(scale: 1.15, duration: const Duration(milliseconds: 320), curve: Curves.elasticOut, child: emoji);
            }
