...
class MoodButton extends StatelessWidget {
  ...
  const MoodButton({super.key, required this.text, required this.onPressed, this.backgroundColor, this.textColor});

  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;

    final bgColor = backgroundColor ?? (isDark ? MoodColors.primaryDark : MoodColors.primaryNormal);
    ...
    return ElevatedButton(
      ...
      onPressed: onPressed,
      child: Text(text, style: MoodTextStyles.normal3.copyWith(fontWeight: FontWeight.w500, color: fgColor)),
    );
  }
}
