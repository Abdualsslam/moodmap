...
class LanguageWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.language, color: isDark ? Colors.blue[200] : const Color(0xFF3B82F6), size: 28),
          const SizedBox(width: 18),
          Text(
            AppLocalizations.of(context)!.language,
            style: TextStyle(fontWeight: FontWeight.w600, fontSize: 18, color: isDark ? Colors.white : Colors.black),
          ),
          const Spacer(),
          ToggleButtons(
            borderRadius: BorderRadius.circular(14),
            ...
            fillColor: isDark ? Colors.blue[200]! : MoodColors.primaryNormalActive,
            constraints: const BoxConstraints(minWidth: 66, minHeight: 40),
            isSelected: [settings.language == 'ar', settings.language == 'en'],
            onPressed: (idx) async {
              final newLang = idx == 0 ? 'ar' : 'en';
