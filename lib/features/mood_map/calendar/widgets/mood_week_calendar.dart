...
import 'package:mood_map/features/mood_map/calendar/widgets/mood_detail_dialog.dart';
import 'package:mood_map/features/mood_map/calendar/widgets/mood_edit_dialog.dart';
      ...
      child: Column(
        children: [
          ...
          Row(
            ...
            children: [
              IconButton(onPressed: _prevWeek, icon: Icon(Icons.chevron_left, color: isDark ? colorScheme.primary : MoodColors.badDarker)),
              Text(
                "${monthName(_weekStart.month, context)} ${_weekStart.year}",
              ...
              IconButton(onPressed: _nextWeek, icon: Icon(Icons.chevron_right, color: isDark ? colorScheme.primary : MoodColors.badDarker)),
            ],
          ),
          const SizedBox(height: 8),
          // --- Row الأيام ---
          Row(
            ...
            children: List.generate(7, (i) {
              ...
              final isFuture = dateOnly.isAfter(todayDateOnly);
              final isDisabled = isFuture || isFiltered;
              final color = isDisabled ? disabledColor : provider.getMoodColorForDay(isDark, date.day, date.month, date.year);
              final isToday = dateOnly == todayDateOnly;
                ...
                child: GestureDetector(
                  ...
                  child: Column(
                    children: [
                      Text(
                        weekDays[i],
                        style: TextStyle(fontWeight: FontWeight.w600, color: isDark ? Colors.white54 : const Color(0xFF4E5A65)),
                      ),
                      const SizedBox(height: 7),
                      Container(
                        width: 39,
                        ...
                        decoration: BoxDecoration(
                          ...
                          border:
                              isToday
                                  ? Border.all(color: isDark ? colorScheme.primary : MoodColors.primaryDarker, width: 2)
                                  : Border.all(color: isDark ? Colors.grey[700]! : MoodColors.awfulNormal),
                        ),
                        alignment: Alignment.center,
                        child: Text(
                          '${date.day}',
                          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: isDisabled ? disabledTextColor : dayTextColor),
                        ),
                      ),
                    ],
                  ),
